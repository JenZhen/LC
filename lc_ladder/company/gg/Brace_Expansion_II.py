    ...
    def braceExpansionII(self, E: str) -> List[str]:
        ...
        while scan < len(E):
            ...
            if E[scan] == '{':
                scan += 1
                stack.append('#')
				# append an additional set to avoid extra if statement
                stack.append(set(['']))
                ...
                scan += 1
                if stack:
                    stack.append(set(p1 + p2 for p1 in stack.pop() for p2 in component))
                else:
                ...
                start = scan
                while E[scan].isalpha():
                    scan += 1
                word = E[start : scan]

                stack.append(set(p + word for p in stack.pop()))
