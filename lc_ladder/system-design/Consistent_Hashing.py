...
# https://www.lintcode.com/problem/consistent-hashing/description?_from=ladder&&fromId=8
# Example
        ...
        for i in range(1, n):
            ...
            for j in range(i):
                if results[j][1] - results[j][0] > \
                   results[splitAt][1] - results[splitAt][0]:
                    splitAt = j

            start, end = results[splitAt][0], results[splitAt][1]
            results[splitAt][1] = int((start + end) / 2)
            results.append([int((start + end) / 2) + 1, end, i + 1])
